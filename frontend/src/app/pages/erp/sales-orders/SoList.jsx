...
export default function SoList() {
  ...
  const fetchOrders = async () => {
    ...
    if (soType) params.so_type = soType;
  ...
  useEffect(() => { fetchOrders(); }, [page, search, status, soType, dateRange]);
  ...
  const typeBody = (row) => (
    <Tag value={row.so_type} severity={row.so_type === "MTS" ? "info" : "success"} className="text-xs" />
  );

  const amountBody = (row) => (
    <span className="font-medium">₹{(row.total_amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
  );
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-3 flex flex-wrap gap-3 items-center">
        ...
        <SelectButton value={soType} options={SO_TYPE_OPTIONS}
          onChange={(e) => { setSoType(e.value || ""); setPage(1); }} />
        ...
        <InputText placeholder="Search by SO#, customer..."
      ...
      <DataTable value={orders} loading={loading}
        ...
        <Column field="so_type" header="Type" body={typeBody} style={{ width: "80px" }} />
        <Column field="line_count" header="Lines" style={{ width: "60px" }} />
        <Column field="total_amount" header="Amount" body={amountBody} style={{ width: "120px" }} />
        <Column field="delivery_date" header="Delivery" style={{ width: "100px" }} />
