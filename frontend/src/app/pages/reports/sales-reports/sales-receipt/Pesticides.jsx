...
// Transform Pesticides API response to component format
const transformPesticidesApiResponse = (apiData) => {
  const { locationData, orderData, itemdetails } = apiData;

  // Transform pesticides items
  const transformedProducts = (itemdetails.pesticide || []).map(
    (item, index) => ({
      srNo: index + 1,
      name: item.itemname,
  ...
  return {
    ...
    PesticidesLicenseNumber: locationData.pesticideslicensenumber || "",
    PesticidesLicenseDate: locationData.pesticideslicensedate || "",
...
export default function Pesticides() {
  ...
  const { billData, isLoading, error, retryFetch } = useBillData(
    "pesticides",
    id,
    transformPesticidesApiResponse,
  );
        ...
        <div className="min-h-screen bg-gray-100 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4">
            ...
            >
              ...
              <div className="p-6">
                {/* Table Header */}
                <div className="mb-2 grid grid-cols-9 gap-2 rounded bg-blue-50 p-2">
                  {[...Array(9)].map((_, i) => (
                    <div
                      key={i}
                ...
                {[...Array(3)].map((_, rowIndex) => (
                  <div
                    key={rowIndex}
                    className="grid grid-cols-9 gap-2 border-b border-gray-100 p-2"
                  >
                    {[...Array(9)].map((_, colIndex) => (
                      <div
                        ...
                        style={{
                          animationDelay: `${(rowIndex * 9 + colIndex) * 50}ms`,
                        }}
                      ></div>
                    ))}
                  </div>
                          ...
                          <tbody>
                            <tr>
                              <td className="w-1/3 p-2 text-left align-top text-sm wrap-break-word">
                                ...
                                <div className="mb-1 text-sm">
                                  <span className="font-bold">દવા લા. નં:</span>{" "}
                                  {billData.PesticidesLicenseNumber || "—"}
                                </div>
                                <div className="mb-1 text-sm">
                                  <span className="font-bold">દવા લા. તા:</span>{" "}
                                  {billData.PesticidesLicenseDate
                                    ? new Date(
                                        billData.PesticidesLicenseDate,
