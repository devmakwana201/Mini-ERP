...
export default function WorkCenterManagement() {
  ...
  const columnOptions = [
    ...
    { field: "capacity_per_day", header: "Capacity (hrs/day)" },
    { field: "cost_per_hour", header: "Cost/Hr" },
    { field: "is_active", header: "Status" },
    { field: "created_at", header: "Created At" },
  ];

  const [visibleFields, setVisibleFields] = useState(() => {
    const saved = sessionStorage.getItem("workCenterList_visibleFields");
    if (saved) {
  ...
  const validateForm = () => {
    ...
    if (formData.capacity_per_day < 0) errors.capacity_per_day = "Must be ≥ 0";
    if (formData.cost_per_hour < 0) errors.cost_per_hour = "Must be ≥ 0";
    setFormErrors(errors);
    return Object.keys(errors).length === 0;
  };

  const resetForm = () => {
    setFormData({ name: "", code: "", description: "", capacity_per_day: 8.0, cost_per_hour: 0.0, is_active: true });
    setFormErrors({});
  };
  ...
  const fetchList = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await WorkCenterService.getFormattedWorkCenters({
        filters,
  ...
  const save = async () => {
    ...
    try {
      const response = editId
        ? await WorkCenterService.updateWorkCenter(editId, formData)
        : await WorkCenterService.createWorkCenter(formData);
      ...
      if (response.success) {
        ...
        toast.current?.show({
          ...
          detail: response.error?.details?.[0]?.message || response.message || "Failed to save",
          life: 3000,
        });
      }
    } catch (error) {
  ...
  const handleEdit = async (rowData) => {
    setFormLoading(true);
    setEditId(rowData.work_center_id);
    setDialog(true);
    try {
      const response = await WorkCenterService.getWorkCenterById(rowData.work_center_id);
      if (response.success) {
        ...
        setFormData({
          ...
          capacity_per_day: d.capacity_per_day ?? 8.0,
          cost_per_hour: d.cost_per_hour ?? 0.0,
  ...
  const confirmDelete = (rowData) => {
    setDeleteId(rowData.work_center_id);
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await WorkCenterService.deleteWorkCenter(deleteId);
      if (res.success) {
        ...
        toast.current?.show({
          severity: "error", summary: "Error",
          detail: res.error?.details?.[0]?.message || res.message || "Failed to delete",
          life: 3000,
        });
      }
    } catch (error) {
  ...
  const exportCSV = () => {
    ...
    link.href = url;
    link.setAttribute("download", "work-centers.csv");
  ...
  const onColumnToggle = (e) => {
    ...
    sessionStorage.setItem("workCenterList_visibleFields", JSON.stringify(ordered.map((c) => c.field)));
  };
  ...
  const header = (
    <div className="flex w-full flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <h3 className="text-sm font-semibold sm:text-base lg:text-lg">Work Center List</h3>
  ...
  return (
    <Page title="Work Center Management">
      <Toast ref={toast} />
          ...
          <div className="col-span-12">
            <div className="prime-card">
              <DataTable
                ...
                dataKey="work_center_id"
                emptyMessage={<EmptyMessage />}
                className="overflow-hidden rounded-lg border border-gray-300"
              >
                {visibleFields.some((c) => c.field === "name") && (
                  <Column field="name" header="Name" sortable body={textTemplate("name")} />
                )}
                {visibleFields.some((c) => c.field === "code") && (
                  <Column field="code" header="Code" sortable body={textTemplate("code")} />
                )}
                {visibleFields.some((c) => c.field === "capacity_per_day") && (
                  <Column field="capacity_per_day" header="Capacity (hrs/day)" sortable body={textTemplate("capacity_per_day")} />
                )}
                {visibleFields.some((c) => c.field === "cost_per_hour") && (
                  <Column field="cost_per_hour" header="Cost/Hr" sortable body={textTemplate("cost_per_hour")} />
                )}
                {visibleFields.some((c) => c.field === "is_active") && (
                  <Column field="is_active" header="Status" body={statusTemplate} />
                )}
                {visibleFields.some((c) => c.field === "created_at") && (
                  <Column field="created_at" header="Created At" sortable body={dateTemplate} />
                )}
      ...
      {/* Add/Edit Dialog */}
      <Dialog header={editId ? "Edit Work Center" : "Add Work Center"} visible={dialog} onHide={hideDialog}
        style={{ width: "480px" }} footer={dialogFooter} draggable={false}>
        {formLoading ? (
          ...
          <div className="flex flex-col gap-4 pt-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <InputText value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))}
                placeholder="e.g. Welding Station" className={formErrors.name ? "p-invalid" : ""} />
            ...
            <div className="flex flex-col gap-1">
              ...
              <InputText value={formData.code} onChange={(e) => setFormData((p) => ({ ...p, code: e.target.value }))}
                placeholder="e.g. WC-001" className={formErrors.code ? "p-invalid" : ""} />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium">Capacity (hrs/day)</label>
                <InputNumber value={formData.capacity_per_day} onValueChange={(e) => setFormData((p) => ({ ...p, capacity_per_day: e.value ?? 0 }))}
                  min={0} max={24} mode="decimal" minFractionDigits={2} maxFractionDigits={2}
                  className={formErrors.capacity_per_day ? "p-invalid" : ""} />
                {formErrors.capacity_per_day && <small className="text-red-500">{formErrors.capacity_per_day}</small>}
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-sm font-medium">Cost per Hour</label>
                <InputNumber value={formData.cost_per_hour} onValueChange={(e) => setFormData((p) => ({ ...p, cost_per_hour: e.value ?? 0 }))}
                  min={0} mode="currency" currency="INR" locale="en-IN"
                  className={formErrors.cost_per_hour ? "p-invalid" : ""} />
              </div>
            </div>
            {/* Active */}
      ...
      <Dialog header="Confirm Delete" visible={deleteDialog} onHide={() => setDeleteDialog(false)}
        ...
        <p>Are you sure you want to delete this Work Center? This action cannot be undone.</p>
      </Dialog>
    </Page>
  );
}
