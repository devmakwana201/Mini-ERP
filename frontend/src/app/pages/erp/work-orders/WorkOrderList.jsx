...
export default function WorkOrderList() {
  ...
  useEffect(() => { fetchWorkOrders(); }, [page, search, status, dateRange]);

  const statusBody = (row) => {
    const severityMap = { pending: "info", in_progress: "warning", completed: "success", hold: "danger" };
  ...
  const priorityBody = (row) => {
    ...
    return <Tag value={row.priority || "medium"} severity={priorityMap[row.priority] || "secondary"} className="text-xs" />;
  };
    ...
    <div className="p-4">
      ...
      <div className="mb-4 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{total} total — auto-generated from MOs</p>
        </div>
      </div>
      ...
      <div className="mb-3 flex flex-wrap gap-3 items-center">
        ...
        <InputText placeholder="Search by WO#, operation, equipment..."
          value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
          className="w-80" />
      </div>

      <DataTable value={workOrders} loading={loading}
        ...
        <Column field="standard_minutes" header="Std. Time (min)" style={{ width: "100px" }} />
        <Column header="Priority" body={priorityBody} style={{ width: "80px" }} />
      ...
      <div className="mt-3 flex gap-2 items-center text-sm text-gray-500">
        ...
        <span>Page {page}</span>
        <Button icon="pi pi-chevron-right" size="small" text disabled={workOrders.length < 20}
