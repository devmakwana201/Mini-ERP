...
export default function FinanceInvoicePage() {
  ...
  const [visibleFields, setVisibleFields] = useState(() => {
    const saved = sessionStorage.getItem("financeInvoice_visibleFields");
    if (saved) {
  ...
  const [dropdownOptions, setDropdownOptions] = useState({ statusOptions: [] });
  ...
  const fetchInvoices = useCallback(() => {
    try {
      ...
      setInvoiceList(filtered.slice(lazyParams.first, lazyParams.first + lazyParams.rows));
    } catch (error) {
      toast.current?.show({ severity: "error", summary: "Error", detail: error.message || "Failed to load invoices", life: 3000 });
      setInvoiceList([]);
      setTotalRecords(0);
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    const ss = sessionStorage.getItem("financeInvoiceTableFilters");
    if (ss) {
  ...
  useEffect(() => {
    const invoiceNumber = location.state?.openInvoice;
    if (!invoiceNumber || allDataRef.current.length === 0) return;
    const invoice = allDataRef.current.find((r) => r.invoiceNumber === invoiceNumber);
    if (invoice) {
      setSelectedInvoice(invoice);
      setPaymentAmount(String(Number(invoice.pending || 0)));
      setPaymentMode("UPI");
  ...
  const handleRecordPayment = () => {
    ...
    setSelectedInvoice((prev) => prev ? { ...prev, paid, pending, status: newStatus } : prev);
    allDataRef.current = allDataRef.current.map((row) => (row.id || row.invoiceNumber) === (selectedInvoice.id || selectedInvoice.invoiceNumber) ? { ...row, paid, pending, status: newStatus } : row);
    fetchInvoices();
    try {
    ...
    toast.current?.show({ severity: "success", summary: "Success", detail: `₹${amount.toFixed(2)} via ${paymentMode} recorded. ${pending > 0 ? `Remaining: ₹${pending.toFixed(2)}` : "Fully paid!"}`, life: 3000 });
    setIsRecordingPayment(false);
    if (pending <= 0) setIsDetailOpen(false);
    else setPaymentAmount(String(pending));
  };

  const formatCurrency = (value) => `₹${value != null ? Number(value).toFixed(2) : "0.00"}`;

  const blankRow = { invoiceNumber: "", soNumber: "", buyerName: "", supplierName: "", total: "", paid: "", pending: "", status: "" };
  ...
  const sanitizeFilterValue = (type, value) => {
    ...
    if (type === "numeric") {
      ...
      if (parts.length > 2) sanitized = `${parts.shift()}.${parts.join("")}`;
      return sanitized;
    }
  ...
  const exportCSV = () => {
    ...
    const link = document.createElement("a"); link.href = url; link.setAttribute("download", "finance_invoice.csv"); document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
    fileExportMessage();
  };
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      doc.save("finance_invoice.pdf"); fileExportMessage();
  ...
  const exportExcel = () => {
    ...
    import("xlsx").then((xlsx) => {
      ...
      import("file-saver").then((mod) => { if (mod?.default) { mod.default.saveAs(new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8" }), `finance_invoice_${new Date().getTime()}.xlsx`); } });
      fileExportMessage();
    });
  };
  ...
  const onColumnToggle = (event) => {
    ...
    sessionStorage.setItem("financeInvoice_visibleFields", JSON.stringify(ordered.map((c) => c.field)));
  };
  ...
  const createValidatedFilterElement = (placeholder, type = "text") => {
    ...
    Elem.displayName = `FIFilter(${placeholder})`;
    return Elem;
  };
  ...
  const createDropdownFilterElement = (placeholder, optionsList) => {
    ...
    Elem.displayName = `FIDropdown(${placeholder})`;
    return Elem;
  };

  // Body templates
  const invoiceNumberBody = (rowData) => isLoading ? <Skeleton width="80%" height="1.5rem" /> : (
    <button type="button" onClick={() => openInvoiceDetails(rowData)} className="cursor-pointer font-semibold text-blue-600 hover:underline">{rowData.invoiceNumber || "-"}</button>
  );
  ...
  const statusBody = (rowData) => isLoading ? <Skeleton width="70%" height="1.5rem" /> : (
    <span className={`rounded-full px-2 py-1 text-xs font-medium ${rowData.status === "Paid" ? "bg-green-100 text-green-800" : rowData.status === "Partial" ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"}`}>{rowData.status || "Unpaid"}</span>
  );
  ...
  const renderHeader = () => (
    <div className="flex w-full flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <h3 className="text-sm font-semibold sm:text-base lg:text-lg">Finance Invoice Report</h3>
  ...
  return (
    <Page title="Finance Invoice">
        ...
        <div className="grid grid-cols-12 gap-4 sm:gap-5 lg:gap-6">
          <div className="col-span-12">
            <div className="prime-card">
              <DataTable
                value={isLoading ? Array.from({ length: 10 }, () => blankRow) : invoiceList}
                className="overflow-hidden rounded-lg border border-gray-300"
                header={renderHeader()}
                emptyMessage={<EmptyMessage title="No invoices found" subtitle="Try adjusting your search criteria." />}
                paginator
                lazy
                filterDisplay="row"
                filterDelay={0}
                filters={filters}
                globalFilterFields={["invoiceNumber", "soNumber", "buyerName", "supplierName", "total", "paid", "pending", "status"]}
                ...
                stateKey="financeInvoiceTableFilters"
                ...
                footerColumnGroup={!isLoading && (
                  <ColumnGroup><Row>
                    <Column footer={`Total Invoices: ${allDataRef.current.length}`} className="font-bold" />
              ...
              >
                ...
                {visibleFields.some((c) => c.field === "soNumber") && <Column field="soNumber" header="SO #" style={{ minWidth: "12rem" }} body={soNumberBody} filter showFilterMenu={false} filterElement={createValidatedFilterElement("Search SO", "text")} sortable />}
                {visibleFields.some((c) => c.field === "buyerName") && <Column field="buyerName" header="Buyer" style={{ minWidth: "12rem" }} body={buyerNameBody} filter showFilterMenu={false} filterElement={createValidatedFilterElement("Search Buyer", "text")} sortable />}
                ...
                {visibleFields.some((c) => c.field === "action") && <Column field="action" header="Action" style={{ minWidth: "18rem" }} body={actionBody} />}
              </DataTable>
            </div>
          </div>
        </div>
      ...
      >
        {!selectedInvoice ? (
          <div className="flex min-h-[240px] items-center justify-center"><Skeleton width="100%" height="200px" /></div>
        ) : (
          <div className="space-y-4">
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="border-b border-gray-200 text-left dark:border-dark-500/40">
                    <th className="px-3 py-2">ITEM</th><th className="px-3 py-2">BATCH</th><th className="px-3 py-2">QTY</th>
                    <th className="px-3 py-2">PRICE</th><th className="px-3 py-2">DISCOUNT</th><th className="px-3 py-2">TAX</th><th className="px-3 py-2">TOTAL</th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm md:grid-cols-5">
              <div className="rounded-md border border-gray-200 px-3 py-2 dark:border-dark-500/40"><p className="text-xs text-gray-500 dark:text-dark-300">SUBTOTAL</p><p className="font-medium">{formatCurrency(selectedInvoice.subtotal)}</p></div>
              <div className="rounded-md border border-gray-200 px-3 py-2 dark:border-dark-500/40"><p className="text-xs text-gray-500 dark:text-dark-300">DISCOUNT</p><p className="font-medium">{formatCurrency(selectedInvoice.discount)}</p></div>
              <div className="rounded-md border border-gray-200 px-3 py-2 dark:border-dark-500/40"><p className="text-xs text-gray-500 dark:text-dark-300">TAX</p><p className="font-medium">{formatCurrency(selectedInvoice.tax)}</p></div>
              <div className="rounded-md border border-gray-200 px-3 py-2 dark:border-dark-500/40"><p className="text-xs text-gray-500 dark:text-dark-300">TCS</p><p className="font-medium">{formatCurrency(selectedInvoice.tcs)}</p></div>
              <div className="rounded-md bg-amber-50 px-3 py-2 text-amber-800 dark:bg-amber-900/20 dark:text-amber-200"><p className="text-xs">GRAND TOTAL</p><p className="font-semibold">{formatCurrency(selectedInvoice.grandTotal)}</p></div>
            </div>
