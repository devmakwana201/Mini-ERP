...
// Transform Fertilizers API response to component format
const transformFertilizersApiResponse = (apiData) => {
  const { locationData, orderData, itemdetails } = apiData;

  // Transform fertilizer items
  const transformedProducts = (itemdetails.fertilizer || []).map(
    (item, index) => ({
      srNo: index + 1,
      name: item.itemname,
      hsnCode: item.hsnseccode,
      packingDetails: "-", // Not provided in API response
      batchLotDate: item.batchinfo || "-",
      qty: `${item.quantity}`,
  ...
  return {
    ...
    FertilizerLicenseNumber: locationData.fertilizerlicensenumber || "",
    FertilizerLicenseDate: locationData.fertilizerlicensedate || "",
    ...
    totaltaxamount: parseFloat(orderData.totaltaxamount) || 0,
    SGST: parseFloat(
      ((parseFloat(orderData.totaltaxamount) || 0) / 2).toFixed(5),
    ),
    CGST: parseFloat(
      ((parseFloat(orderData.totaltaxamount) || 0) / 2).toFixed(5),
    ),
...
export default function Fertilizers() {
  ...
  const { billData, isLoading, error, retryFetch } = useBillData(
    "fertilizers",
    id,
    transformFertilizersApiResponse,
  );
        ...
        <div className="min-h-screen bg-gray-100 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4">
            ...
            >
              ...
              <div className="p-6">
                {/* Table Header */}
                <div className="mb-2 grid grid-cols-8 gap-2 rounded bg-blue-50 p-2">
                  {[...Array(8)].map((_, i) => (
                    <div
                      key={i}
                ...
                {[...Array(3)].map((_, rowIndex) => (
                  <div
                    key={rowIndex}
                    className="grid grid-cols-8 gap-2 border-b border-gray-100 p-2"
                  >
                          ...
                          <tbody>
                            <tr>
                              <td className="w-1/3 p-2 text-left align-top text-sm wrap-break-word">
                                ...
                                <div className="mb-1 text-sm">
                                  <span className="font-bold">ખા. લા. નં:</span>{" "}
                                  {billData.FertilizerLicenseNumber || "—"}
                                </div>
                                <div className="mb-1 text-sm">
                                  <span className="font-bold">ખા. લા. તા:</span>{" "}
                                  {billData.FertilizerLicenseDate
                                    ? new Date(
                                        billData.FertilizerLicenseDate,
